...
class VerifyC2DescribeCoreComponentState : public VerifyC2Describe {
 ...
 protected:
  void updateProperties(org::apache::nifi::minifi::utils::debug_shared_ptr<minifi::FlowController> flow_controller) override {
    dynamic_pointer_cast<minifi::state::ProcessorController>(flow_controller->getComponents("TailFile1")[0])
        ->getProcessor()->setProperty(minifi::processors::TailFile::FileName, test_file_1_);
    dynamic_pointer_cast<minifi::state::ProcessorController>(flow_controller->getComponents("TailFile2")[0])
