...
void JoinEnrichmentAttributes::join(std::shared_ptr<core::FlowFile> one, std::shared_ptr<core::FlowFile> two,
    core::ProcessSession& session, const EnrichmentRole role_one) const {
  auto [original, enrichment] = (role_one == EnrichmentRole::ORIGINAL) ? std::tie(one, two) : std::tie(two, one);
...
void JoinEnrichmentAttributes::handleFlowFile(std::shared_ptr<core::FlowFile> flow_file, core::ProcessSession& session,
  ...
  if (const auto pair_node = pair_map.extract(group_id)) {
    ...
    join(std::move(flow_file), std::move(pair_node.mapped()), session, *role);
  } else {
