...
for %%x in (%*) do (
    ...
    if [%%~x] EQU [/O]           set build_opencv=ON & set with_opencv=True
    ...
    if [%%~x] EQU [/CONANONLY]   set conanonly=ON
    if [%%~x] EQU [/SKIPCONAN]   set skipconan=ON
)

mkdir %builddir%
pushd %builddir%\

if [%skipconan%] NEQ [ON] conan install .. -o with_opencv=%with_opencv% --build=opencv --build=rocksdb
if [%conanonly%] EQU [ON] exit /b %ERRORLEVEL%
