...
TEST_CASE("Flow shutdown drains connections", "[TestFlow1]") {
  ...
  auto sinkProc = static_pointer_cast<minifi::processors::TestProcessor>(root->findProcessor("TestProcessor"));
...
TEST_CASE("Flow shutdown waits for a while", "[TestFlow2]") {
  ...
  auto sourceProc = static_pointer_cast<minifi::processors::TestFlowFileGenerator>(root->findProcessor("Generator"));
  auto sinkProc = static_pointer_cast<minifi::processors::TestProcessor>(root->findProcessor("TestProcessor"));
